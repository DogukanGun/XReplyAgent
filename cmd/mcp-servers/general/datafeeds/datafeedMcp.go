...
// PythMetadata represents metadata from Pyth Network
...
// PythParsedPrice represents a parsed price from Pyth Network
...
// PythResponse represents the response from Pyth Hermes API
type PythResponse struct {
	Data []PythParsedPrice `json:"parsed"`
}

// DataFeedServer represents the MCP server for data feeds
type DataFeedServer struct {
	mcpServer *server.MCPServer
}

// NewDataFeedServer creates a new data feed MCP server
func NewDataFeedServer() *DataFeedServer {
...
func (dfs *DataFeedServer) registerTools() {
	// Pyth Network Tools
	dfs.registerPythTools()

	// Chainlink Tools
	dfs.registerChainlinkTools()
}

// registerPythTools registers Pyth Network related tools
func (dfs *DataFeedServer) registerPythTools() {
	// Tool: Get Pyth price feeds (all hardcoded feeds)
	pythAllFeedsTools := mcp.Tool{
	...
	dfs.mcpServer.AddTool(pythAllFeedsTools, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		feeds, err := dfs.fetchAllPythPriceFeeds()
		if err != nil {
	...
	pythSpecificFeedsTools := mcp.Tool{
	...
	dfs.mcpServer.AddTool(pythSpecificFeedsTools, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		feeds, err := dfs.fetchSpecificPythPriceFeeds(assets)
		if err != nil {
	...
	dfs.mcpServer.AddTool(pythListFeedsTools, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
// registerChainlinkTools registers Chainlink related tools
func (dfs *DataFeedServer) registerChainlinkTools() {
	// Tool: Get Chainlink price feed
	chainlinkPriceTools := mcp.Tool{
	...
	dfs.mcpServer.AddTool(chainlinkPriceTools, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		price, err := dfs.getChainlinkPrice(chain, asset)
		if err != nil {
	...
	dfs.mcpServer.AddTool(chainlinkInfoTools, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func (dfs *DataFeedServer) Start() error {
...
// Helper functions for Pyth Network integration
...
func (dfs *DataFeedServer) fetchAllPythPriceFeeds() (*PythResponse, error) {
	var feedIDs []string
	for _, id := range pythPriceFeedIDs {
		feedIDs = append(feedIDs, id)
	}
	return dfs.fetchPythPriceFeeds(feedIDs)
}

func (dfs *DataFeedServer) fetchSpecificPythPriceFeeds(names []string) (*PythResponse, error) {
	var feedIDs []string
...
func (dfs *DataFeedServer) fetchPythPriceFeeds(feedIDs []string) (*PythResponse, error) {
...
func (dfs *DataFeedServer) listAvailablePythFeeds() map[string]interface{} {
...
// Helper functions for Chainlink integration

func (dfs *DataFeedServer) getChainlinkPrice(chainName, assetName string) (float64, error) {
	// This calls the existing chainlink package function
	return chainlink.GetPriceFromChainlink(chainName, assetName)
}
...
// CreateDataFeedMcpServer creates and starts the data feed MCP server
func CreateDataFeedMcpServer() {
	server := NewDataFeedServer()

	if err := server.Start(); err != nil {
		log.Fatalf("Failed to start DataFeed MCP server: %v", err)
	}
}
