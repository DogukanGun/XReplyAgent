...
func TestGetErc20TokenTransferForAddress(t *testing.T) {
	...
	t.Run("Test GenerateErc20TokenTransferTool", func(t *testing.T) {
		tool, handler := ge.GenerateErc20TokenTransferTool()

		// Verify tool properties
		if tool.Name != "get_erc20_token_transfer_for_address" {
			t.Errorf("Expected tool name 'get_erc20_token_transfer_for_address', got '%s'", tool.Name)
		}
...
func TestErc20TokenTransferToolParameters(t *testing.T) {
	os.Setenv("GOLDRUSH_AUTH_TOKEN", "test_token")
	defer os.Unsetenv("GOLDRUSH_AUTH_TOKEN")

	ge := endpoints.NewGoldrushEndpoints()
	tool, _ := ge.GenerateErc20TokenTransferTool()
