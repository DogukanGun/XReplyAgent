...
func TestGetNftsForAddress(t *testing.T) {
	...
	t.Run("Test GenerateNftsForAddressTool", func(t *testing.T) {
		tool, handler := ge.GenerateNftsForAddressTool()

		// Verify tool properties
		if tool.Name != "get_nfts_for_address" {
			t.Errorf("Expected tool name 'get_nfts_for_address', got '%s'", tool.Name)
		}
...
func TestNftsForAddressToolParameters(t *testing.T) {
	os.Setenv("GOLDRUSH_AUTH_TOKEN", "test_token")
	defer os.Unsetenv("GOLDRUSH_AUTH_TOKEN")

	ge := endpoints.NewGoldrushEndpoints()
	tool, _ := ge.GenerateNftsForAddressTool()
