...
func TestGetTokenBalanceForAddress(t *testing.T) {
	...
	t.Run("Test GenerateTokenBalanceForAddressTool", func(t *testing.T) {
		tool, handler := ge.GenerateTokenBalanceForAddressTool()

		// Verify tool properties
		if tool.Name != "get_token_balance_for_address" {
			t.Errorf("Expected tool name 'get_token_balance_for_address', got '%s'", tool.Name)
		}
...
func TestTokenBalanceForAddressToolParameters(t *testing.T) {
	os.Setenv("GOLDRUSH_AUTH_TOKEN", "test_token")
	defer os.Unsetenv("GOLDRUSH_AUTH_TOKEN")

	ge := endpoints.NewGoldrushEndpoints()
	tool, _ := ge.GenerateTokenBalanceForAddressTool()
