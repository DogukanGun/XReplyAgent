...
func main() {
	// Upstream BNB MCP over SSE
	upstream := os.Getenv("SOLANA_MCP_SSE")
	if upstream == "" {
		log.Fatal("SOLANA_MCP_SSE is required (e.g., http://localhost:3001/sse)")
	}
	...
	if _, err := mcpCl.Initialize(ctx, mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			ClientInfo:      mcp.Implementation{Name: "solana-proxy", Version: "0.1.0"},
		},
	}); err != nil {
	...
	s := server.NewMCPServer("solana-proxy", "0.1.0", server.WithToolCapabilities(true), server.WithLogging())
	...
	log.Printf("solana-proxy MCP server listening on :%s/mcp (forwarding to SSE %s)", port, upstream)
