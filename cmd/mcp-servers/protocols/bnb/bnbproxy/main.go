...
func main() {
	...
	upstream := os.Getenv("BNB_MCP_SSE")
	if upstream == "" {
		log.Fatal("BNB_MCP_SSE is required (e.g., http://localhost:3001/sse)")
	}
	...
	if _, err := mcpCl.Initialize(ctx, mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			ClientInfo:      mcp.Implementation{Name: "bnb-proxy", Version: "0.1.0"},
		},
	}); err != nil {
	...
	s := server.NewMCPServer("bnb-proxy", "0.1.0", server.WithToolCapabilities(true), server.WithLogging())
	...
	log.Printf("bnb-proxy MCP server listening on :%s/mcp (forwarding to SSE %s)", port, upstream)
